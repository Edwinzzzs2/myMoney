...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (hardDelete) {
      const usageRows = await query('SELECT COUNT(*)::int AS count FROM my_money_expenses WHERE category_id = $1', [id])
      ...
      const deleteResult = await execute('DELETE FROM my_money_categories WHERE id = $1 AND is_active = FALSE', [id])
