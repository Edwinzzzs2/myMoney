...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await execute(
      'UPDATE my_money_expenses SET trip_id = $1, category_id = $2, amount = $3, title = $4, merchant = $5, expense_date = $6, expense_time = NULLIF($7, \'\')::time, payment_method = $8, invoice_status = $9, reimbursement_status = $10, reimbursable = $11, note = $12, receipt_url = $13, updated_at = now() WHERE id = $14',
      [
        payload.tripId,
        payload.categoryId,
        payload.amount,
    ...
    const rows = await query(`${expenseSelect} WHERE e.id = $1`, [id])
...
export async function DELETE(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await execute('DELETE FROM my_money_expenses WHERE id = $1', [id])
    if (result.rowCount === 0) {
      return NextResponse.json({ message: '账单不存在' }, { status: 404 })
    }
