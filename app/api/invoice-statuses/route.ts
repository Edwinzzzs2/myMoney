...
export async function GET() {
  try {
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '获取发票状态失败') }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '保存发票状态失败') }, { status: 400 })
  }
}
