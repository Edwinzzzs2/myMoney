...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!Number.isInteger(id)) return NextResponse.json({ message: '无效支付方式 ID' }, { status: 400 })
    ...
    return NextResponse.json({ message: e.message || '更新支付方式失败' }, { status: 400 })
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ message: e.message || '停用支付方式失败' }, { status: 500 })
  }
}
