...
export async function GET() {
  try {
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '获取支付方式失败') }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '保存支付方式失败') }, { status: 400 })
  }
}
