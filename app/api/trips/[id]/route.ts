...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!Number.isInteger(id)) return NextResponse.json({ message: '无效行程编号' }, { status: 400 })
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '更新行程失败') }, { status: 400 })
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ message: friendlyErrorMessage(e, '归档行程失败') }, { status: 500 })
  }
}
